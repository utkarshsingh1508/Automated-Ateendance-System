...
def insertOrUpdate(Id, Name, roll) :                                            # this function is for database
    connect = sqlite3.connect("Face-DataBase3")                                  # connecting to the database
    ...
    if isRecordExist == 1:                                                      # updating name and roll no
        ...
        connect.execute("UPDATE Students SET Presents = ? WHERE ID = ?",(5,Id))
        time = datetime.now()
        connect.execute("UPDATE Students SET LastUpdatedTime = ? WHERE ID = ?",(time,Id))
    else:
    	params = (Id, Name, roll)                                               # insering a new student data
    	connect.execute("INSERT INTO Students VALUES(?, ?, ?,0,'now')", params)
